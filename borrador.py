...
class TaskManagerApp:
    def __init__(self, root):
        ...
        self.root.geometry("800x600")  # Tamaño de la ventana reducido
        self.root.configure(bg="#2c2f38")

        self.header = tk.Label(
            root, text="Gestor de Tareas", font=("Segoe UI", 16, "bold"),  # Tamaño de fuente reducido
            bg="#1f2328", fg="#f0f0f0", pady=10)
        self.header.pack(fill=tk.X)

        self.form_frame = tk.Frame(root, bg="#2c2f38")
        self.form_frame.pack(pady=10)
        self.titulo_label = tk.Label(self.form_frame, text="Título:", bg="#2c2f38", font=("Segoe UI", 10), fg="#f0f0f0")
        self.titulo_label.grid(row=0, column=0, padx=5, pady=5, sticky=tk.W)
        self.titulo_entry = tk.Entry(self.form_frame, width=30, font=("Segoe UI", 10), bg="#3d4149", fg="#f0f0f0", insertbackground="white")
        self.titulo_entry.grid(row=0, column=1, padx=5, pady=5)

        self.descripcion_label = tk.Label(self.form_frame, text="Descripción:", bg="#2c2f38", font=("Segoe UI", 10), fg="#f0f0f0")
        self.descripcion_label.grid(row=1, column=0, padx=5, pady=5, sticky=tk.W)
        self.descripcion_entry = tk.Entry(self.form_frame, width=30, font=("Segoe UI", 10), bg="#3d4149", fg="#f0f0f0", insertbackground="white")
        self.descripcion_entry.grid(row=1, column=1, padx=5, pady=5)

        self.add_button = tk.Button(
            self.form_frame, text="Agregar Tarea", bg="#4CAF50", fg="white", font=("Segoe UI", 10, "bold"),
            relief="raised", command=self.agregar_tarea)
        self.add_button.grid(row=2, column=0, columnspan=2, pady=10)

        self.list_frame = tk.Frame(root, bg="#2c2f38")
        self.list_frame.pack(pady=10)
        self.tareas_listbox = tk.Listbox(self.list_frame, width=50, height=10, font=("Segoe UI", 10), bg="#3d4149", fg="#f0f0f0", selectbackground="#4CAF50", selectforeground="white")
        ...
        self.complete_button = tk.Button(
            self.actions_frame, text="Completar Tarea", bg="#FF9800", fg="white", font=("Segoe UI", 10, "bold"),
            relief="raised", command=self.completar_tarea)
        self.complete_button.grid(row=0, column=0, padx=10, pady=5)

        self.delete_button = tk.Button(
            self.actions_frame, text="Eliminar Tareas Completadas", bg="#F44336", fg="white", font=("Segoe UI", 10, "bold"),
            relief="raised", command=self.eliminar_tareas_completadas)
        self.delete_button.grid(row=0, column=1, padx=10, pady=5)

        self.export_button = tk.Button(
            self.actions_frame, text="Exportar Tareas", bg="#2196F3", fg="white", font=("Segoe UI", 10, "bold"),
            relief="raised", command=self.exportar_tareas)
        self.export_button.grid(row=0, column=2, padx=10, pady=5)

        self.import_button = tk.Button(
            self.actions_frame, text="Importar Tareas", bg="#2196F3", fg="white", font=("Segoe UI", 10, "bold"),
            relief="raised", command=self.importar_tareas)
        self.import_button.grid(row=0, column=3, padx=10, pady=5)
